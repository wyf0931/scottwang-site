...
export function generateStaticParams() {
  ...
  return research.length > 0 ? research.map((r) => ({ slug: r.slug })) : [{ slug: ".none" }];
}
...
export default async function ResearchDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (slug === ".none" || !report) notFound();
  const content = await renderMdx(report.body);
  const jsonLd = { "@context": "https://schema.org", "@type": "Article", headline: report.title, description: report.description, datePublished: report.date.toISOString(), dateModified: (report.updated ?? report.date).toISOString(), articleSection: report.topic, about: { "@type": "Thing", name: report.industry }, keywords: report.tags, citation: report.sources.map((source) => source.url), isPartOf: { "@type": "CollectionPage", name: "ScottWang Research", url: `${site.url}/resources` }, author: { "@type": "Person", name: "ScottWang", url: site.url }, mainEntityOfPage: `${site.url}${researchPath(report.slug)}` };
  return <article className="research-detail"><header className="research-detail-header"><p className="eyebrow accent">/ resources · research</p><h1>{report.title}</h1><p className="lead">{report.description}</p><div className="research-meta"><span>{report.industry}</span><span>{report.topic}</span><time dateTime={(report.updated ?? report.date).toISOString()}>Updated {(report.updated ?? report.date).toLocaleDateString("zh-CN")}</time></div><a className="raw-link" href={researchMarkdownPath(report.slug)}>Read Markdown ↗</a></header><div className="research-brief"><strong>Research brief</strong><span>Published report · {report.sources.length} sources</span></div><CodeBlockWrapper><div className="article-body">{content}</div></CodeBlockWrapper>{report.sources.length > 0 && <aside className="research-sources"><h2>Sources</h2><ol>{report.sources.map((source) => <li key={source.url}><a href={source.url} target="_blank" rel="noreferrer">{source.title} ↗</a></li>)}</ol></aside>}{report.tags.length > 0 && <aside className="content-taxonomy" aria-label="Research tags"><p className="sidebar-label">Tags</p><div className="tag-row">{report.tags.map((tag) => <Link href={`/content?tag=${encodeURIComponent(tag)}`} key={tag}>#{tag}</Link>)}</div></aside>}<GiscusComments /><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} /></article>;
}
