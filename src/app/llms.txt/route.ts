...
export function GET() {
  ...
  const books = getAllBooks().map((book) => `- [${book.title}](${site.url}/resources#books): ${book.description}\n  Reader: ${book.readerUrl}\n  Source: ${book.sourceUrl}\n  License note: ${book.licenseNote}`).join("\n");
