...
export default async function CollectionPage({ params }: { params: Promise<{ type: string }> }) {
  ...
  return <section className="collection-page"><p className="eyebrow accent">/ {type}</p><h1>{type === "writing" ? "Writing" : type === "notes" ? "Notes" : "Thoughts"}</h1><p className="lead">关于技术、AI、Agent 系统与正在形成的判断。</p><div className="taxonomy-links"><Link href="/tags">Browse tags →</Link><Link href="/series">Browse series →</Link><Link href="/archive">View archive →</Link></div><div className="content-grid">{entries.map((entry) => <ContentCard key={entry.slug} entry={entry} />)}</div></section>;
}
