...
export default function Home() {
  ...
  return <div className="home-page">
    <section className="hero"><p className="eyebrow accent">/ personal system online</p><h1>Build systems<br /><em>that compound.</em></h1><p className="hero-copy">我是王云飞，英文名 ScottWang。互联网技术与 AI / Agent 架构师，记录正在构建的系统、阅读过的资料、值得保存的资源，以及仍在形成的判断。</p><div className="hero-actions"><Link className="button button-primary" href="/content">Explore content <span>↗</span></Link><Link className="text-link" href="/about">About me <span>→</span></Link></div></section>
    ...
    <section className="content-section"><div className="section-heading"><div><p className="eyebrow accent">Latest transmissions</p><h2>Recent thinking</h2></div><Link className="text-link" href="/content">View all <span>→</span></Link></div><div className="content-grid">{content.slice(0, 3).map((entry) => <ContentCard key={`${entry.type}-${entry.slug}`} entry={entry} />)}</div></section>
